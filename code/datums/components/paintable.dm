...
/datum/component/spraycan_paintable/proc/Repaint(datum/source, obj/item/toy/crayon/spraycan/spraycan, mob/living/user)
	...
	if(spraycan.capped)
		to_chat(user, "<span class='warning'>Сперва снимите колпачок!</span>")
		return
	if(spraycan.uses < 2)
		to_chat(user, "<span class ='warning'>Недостаточно краски в баллончике!")
		return
	RemoveCurrentCoat()
	...
	to_chat(user, "<span class='notice'>Вы распрыскали [spraycan] на [A], закрашивая его.</span>")
