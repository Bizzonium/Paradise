...
/datum/component/label/proc/OnAttackby(datum/source, obj/item/attacker, mob/user)
	...
	to_chat(user, "<span class='warning'>Вы сняли этикетку с [parent].</span>")
...
/datum/component/label/proc/Examine(datum/source, mob/user, list/examine_list)
	examine_list += "<span class='notice'>Присутствует этикетка, на которой что-то написано. Используйте этикетировщик чтобы снять её.</span>"
